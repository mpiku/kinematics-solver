...
a=4; b=9; c=1;
A=[0 0]'; B=kinSolver.rot(alpha_1)*u*a; C=B+kinSolver.rot(alpha_3)*u*b; D=B+0.7*(C-B);
E=[7 0]'; F=E+kinSolver.rot(alpha_2)*u*c; G=F+0.3*(C-B); H=C+1.5*(G-C);
...
el_1 = kinSolver.element(C1); el_1.addPoint(A); el_1.addPoint(B);
el_2 = kinSolver.element(C2); el_2.addPoint(E); el_2.addPoint(F);
el_3 = kinSolver.element(C3); el_3.addPoint(B); el_3.addPoint(C); el_3.addPoint(D);
el_4 = kinSolver.element(C4); el_4.addPoint(F); el_4.addPoint(D);
el_5 = kinSolver.element(C5); el_5.addPoint(F); el_5.addPoint(G);
el_6 = kinSolver.element(C6); el_6.addPoint(C); el_6.addPoint(G); el_6.addPoint(H);
...
kinSolver.grapher.trajectory(el_6, H);